...
FilterAnalysis filter::ForwardFilterImpl::decl(const CallBase &current, const Path& p) const {
...
FilterAnalysis filter::ForwardFilterImpl::def(const CallBase &current, const Path& p) const {
  const bool match_sig = matcher->match(current) == Matcher::MatchResult::Match;
  if (match_sig) {
    if (!deep_matcher || deep_matcher->match(current) == Matcher::MatchResult::Match) {
      auto result = correlate2void(current, p);
